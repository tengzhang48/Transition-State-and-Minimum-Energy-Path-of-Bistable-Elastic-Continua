%%% Apply BITSS algorithm to find the saddle point
clear;
clc
...
%%% set up force boundary condition
...
% res2_1 and res2_2 are two images calculated from BITSS method
timeElapsed1 = toc;
%%%
...
triplot(TR,'r');hold on; % plot the first image in BITSS

coord_def(:,1) = res2_2(1:2:end-1);
coord_def(:,2) = res2_2(2:2:end);
TR = triangulation(connect,coord_def);
triplot(TR,'r');hold on; % plot the second image in BITSS
...
%%% Output BITSS results
filename='BuckleBeam_y1_mesh4-50_BITSS_pt1_alpha10_beta01_dist005-005_re1e-8_iter3_dist0.txt';
