...
abaqus_disp = load('Data_Files\BuckleBeam_y1_Right_mesh4-50_Disp.txt'); % displacement from ABAQUS 
xy = coord';
xy = xy + abaqus_disp(:,2:3)';
xy = xy(:);  % re-arranged into a single column vector
...
N_load = 1/load_factor; % loading steps
...
for ii = 1:N_load
    ...
    triplot(TR);axis equal; % plot results at each loading step
end
...
coord_def(:,2) = xy_def(2:2:end,1);
TR = triangulation(connect,coord_def);
triplot(TR); axis equal; % plot final result

%%% Output stable state
filename = 'BuckleBeam_y1_Right_mesh4-50.txt';
