...
xy_1 = load('Data_Files\BuckleBeam_y1_Left_mesh4-50.txt'); % input the first stable state
xy_2 = load('Data_Files\BuckleBeam_y1_Right_mesh4-50.txt'); % input the second stable state
xy = xy_2; % Indentation starts from the buckled-right shape
...
disp_fix_5 = [middle_node,ones(length(middle_node),1),indent.*ones(length(middle_node),1)]; % indent the center along x direction
disp_fix = [disp_fix_1;disp_fix_2;disp_fix_3;disp_fix_4;disp_fix_5];

force_applied = [];
load_factor = 0.001; % loading factor
N_load = 1/load_factor;
...
for ii = 1:N_load
    ...
    % record the beam center's coordinate and total energy
    xindent(ii) = xy_def(2*middle_node-1); 
    energy_indent(ii) = total_energy(xy_def,coord,connect,mu,lambda); 
    figure(1)
...
figure(2)
plot(xindent,energy_indent,'k','LineWidth',2); % plot indentation curve
figure(3)
coord_def(:,1) = xy_def(1:2:end,1);
coord_def(:,2) = xy_def(2:2:end,1);
TR = triangulation(connect,coord_def);
...
%%% output indentation results
filename = 'BuckleBeam_y1_mesh4-50_IndentMiddle_Right-to-Left.txt';
fid = fopen(filename,'wt');
for ii = 1:N_load
    fprintf(fid,'%.12f\n',xindent(ii),energy_indent(ii));
end
fclose(fid);
%%%
