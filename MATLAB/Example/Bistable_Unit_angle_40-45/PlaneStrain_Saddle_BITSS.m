%%% Apply BITSS method to capture the saddle point
clear;
clc
% Input nodal and element informatino
...
u1 = efn(xy_1); % energy of S1
u2 = efn(xy_2); % energy of S2

%%% run
tic;
[res2_1, res2_2] = bitss_hessian(xy_1, xy_2, efn, gfn, hfn);
timeElapsed1 = toc;
%%%

coord_def(:,1) = res2_1(1:2:end-1);
coord_def(:,2) = res2_1(2:2:end);
TR = triangulation(connect,coord_def);
triplot(TR,'r');hold on; % plot first image in BITSS

coord_def(:,1) = res2_2(1:2:end-1);
coord_def(:,2) = res2_2(2:2:end);
TR = triangulation(connect,coord_def);
triplot(TR,'r');hold on; % plot second image in BITSS
...
TR = triangulation(connect,coord_def);
triplot(TR,'k');hold on; % plot S2

%%% BITSS results output
filename = 't06_L5_theta40-45_twobeam_mesh4-50_mu1_lambda3_E100_nu03_BITSS1_alpha10_beta01_dist005-005_iter3_Re1e-5.txt';
